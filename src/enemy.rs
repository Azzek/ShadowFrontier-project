use bevy::{log::tracing_subscriber::filter::targets, prelude::*, state::commands, window::PrimaryWindow};
...
fn spawn_enemy(
    ...
    anim_map.insert(AnimationState::Idle, (idle_tex.clone(), anim_layout.clone()));
    anim_map.insert(AnimationState::Attack, (atk_tex, anim_layout.clone()));
    anim_map.insert(AnimationState::Walk, (walk_tex, anim_layout.clone()));
    anim_map.insert(AnimationState::Hurt, (hurt_tex, hurt_layout.clone()));
...
fn drop_target(
    ...
    for (enemy, target, tf) in query.iter_mut() {
        if let Ok(target_tf) = targets.get(target.target) {
            if tf.translation.distance(target_tf.translation) > 550. {
                commands.entity(enemy).remove::<Target>();
            }
        }
    }
...
fn hit_reaction(
    mut commands: Commands,
    mut query: Query<(Entity, &Stats, &mut HitReactionTimer, &mut Animation), With<Enemy>>,
    time: Res<Time>,
) {
